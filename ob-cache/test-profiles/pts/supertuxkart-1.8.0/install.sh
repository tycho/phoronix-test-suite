#!/bin/sh
rm -rf SuperTuxKart-1.5-linux
if [ $OS_ARCH = "aarch64" ]
then
	tar -xf SuperTuxKart-1.5-linux-arm64.tar.gz
	mv SuperTuxKart-1.5-linux-arm64 SuperTuxKart-1.5-linux
else
	tar -xf SuperTuxKart-1.5-linux-x86_64.tar.gz
	mv SuperTuxKart-1.5-linux-x86_64 SuperTuxKart-1.5-linux
fi
cat > config.xml <<'EOT'
<?xml version="1.0"?>
<stkconfig version="8" >
...
EOT
mkdir -p ~/.config/supertuxkart/config-0.10/
echo "#!/bin/sh
cp -f config.xml ~/.config/supertuxkart/config-0.10/config.xml
rm -f ~/.config/supertuxkart/config-0.10/stdout.log
rm -f ~/.config/supertuxkart/config-0.10/stdout.log.perf-report-black_forest.csv
cd SuperTuxKart-1.5-linux
./run_game.sh \$@ 2>&1
cp ~/.config/supertuxkart/config-0.10/stdout.log \$LOG_FILE
cat ~/.config/supertuxkart/config-0.10/stdout.log.perf-report-black_forest.csv >> \$LOG_FILE" > supertuxkart
