#!/bin/sh
rm -rf mariadb_
tar -xf mariadb-12.3.0.tar.gz
mkdir ~/mariadb_
cd mariadb-12.3.0/BUILD/
...
export PATH=$HOME/mariadb_/bin/:$HOME/mariadb-12.3.0/BUILD/extra/:$PATH
make install
...
sleep 3
sysbench \$1 --threads=\$2 --time=120 --rand-type=uniform --db-driver=mysql --mysql-db=sbtest --mysql-host=localhost --mysql-user=\`basename $DEBUG_REAL_HOME\` --mysql-password=phoronix  --mysql-socket=/tmp/mysql.sock run --tables=16 --table-size=1000000 > \$LOG_FILE 2>&1
