#!/bin/sh
tar -xf gromacs-2025.4.tar.gz
tar -xf water_GMX50_bare.tar.gz
mkdir mpi-build
cd mpi-build
cmake ../gromacs-2025.4 -DGMX_OPENMP=OFF -DGMX_MPI=ON -DGMX_GPU=OFF -DGMX_BUILD_OWN_FFTW=ON -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=ON
...
then
	cd cuda-build
	cmake ../gromacs-2025.4 -DGMX_MPI=OFF -DGMX_OPENMP=ON -DGMX_GPU=CUDA -DGMX_BUILD_OWN_FFTW=ON -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=ON
	make -j $NUM_CPU_CORES
